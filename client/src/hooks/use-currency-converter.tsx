...
export function useCurrencyConverter() {
  ...
  // Default to USD if user has no preference
  const userCurrency = user?.defaultCurrency || 'USD';

  /**
   * Convert an amount from one currency to the user's preferred currency
   */
  const convertToUserCurrency = (amount: number | string, fromCurrency: string = 'USD'): number => {
   ...
   */
  const formatInUserCurrency = (amount: number | string, fromCurrency: string = 'USD'): string => {
